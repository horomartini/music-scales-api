...
import old_db from '../../db/sample-db'
...
import { QueryFilter, QueryHateoas, QueryPaginator, QuerySorter } from 'types/req'
import { NoteDoc } from 'types/db'
import { applyFiltering } from 'middleware/response'
...
router.get('/notes/:note', (
  ...
  const noteDb = old_db.getNote(params.note.name)
...
router.get('/instruments', (
  ...
  const instrumentsDb = old_db.getInstruments()
...
router.get('/instruments/:instrument', (
  ...
  const instrumentDb = old_db.getInstrument(params.instrument, { exactMatch: params.exactMatch })
...
router.get('/tunings', (
  ...
  const tuningsDb = old_db.getTunings()
...
router.get('/tunings/:tuning', (
  ...
  const tuningDb = old_db.getTuning(params.tuning, { exactMatch: params.exactMatch })
...
router.get('/scales', async (
  ...
  const scalesDb = old_db.getScales()

  let scales: (IScale | IScaleExt)[] = scalesDb

  if (params.notes.length > 0) {
    const notesDb = old_db.getNotes()
...
router.get('/scales/:scale', (
  ...
  const scaleDb = old_db.getScale(params.scale, { exactMatch: params.exactMatch })
