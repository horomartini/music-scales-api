...
export const resolvers = {
  Query: {
    notes: async (_: unknown, { filter, sort, paginate }: UXInputs) => {
      let data = await model.Notes.find().exec()
    ...
    note: async (_: unknown, { _id }: { _id: string }) => {
      ...
      let data = await model.Notes.findOne({ _id: new mongoose.Types.ObjectId(_id) })
      return data
    },
  ...
  Instrument: {
    // defaultTuning: async (parent: any) => await db.tunings.getOne({ _id: parent.defaultTuning }),
  },
  Tuning: {
    // instrument: async (parent: any) => await db.instruments.getOne({ _id: parent.instrument }),
  },
  TuningNotes: {
    // note: async (parent: any) => await db.notes.getOne({ _id: parent.note }),
  },
